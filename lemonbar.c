...
void update_gc(void)
{
	...
	xcb_change_gc(c, gc[GC_CLEAR], XCB_GC_FOREGROUND,
		      (const uint32_t[]){ bgc.v });
	xcb_change_gc(c, gc[GC_ATTR], XCB_GC_FOREGROUND,
	       ...
	       int height)
{
	xcb_poly_fill_rectangle(
		c, d, _gc, 1,
		(const xcb_rectangle_t[]){ { x, y, width, height } });
}
					  ...
					  const uint16_t *str)
{
	...
	xcb_parts[6].iov_len =
		-(xcb_parts[4].iov_len + xcb_parts[5].iov_len) & 3;
...
int xft_char_width(uint16_t ch, font_t *cur_font)
{
	...
	if (!xft_char[slot]) {
		XGlyphInfo gi;
		FT_UInt glyph =
			XftCharIndex(dpy, cur_font->xft_ft, (FcChar32)ch);
...
int shift(monitor_t *mon, int x, int align, int ch_width)
{
	switch (align) {
	case ALIGN_C:
	...
	case ALIGN_R:
		xcb_copy_area(c, mon->pixmap, mon->pixmap, gc[GC_DRAW],
			      mon->width - x, 0, mon->width - x - ch_width, 0,
			      x, bh);
...
void draw_shift(monitor_t *mon, int x, int align, int w)
{
...
int draw_char(monitor_t *mon, font_t *cur_font, int x, int align, uint16_t ch)
{
	...
	if (cur_font->xft_ft) {
		XftDrawString16(xft_draw, &sel_fg, cur_font->xft_ft, x, y, &ch,
				1);
	} else {
		...
		xcb_poly_text_16_simple(c, mon->pixmap, gc[GC_DRAW], x, y, 1,
					&ch);
	}
...
rgba_t parse_color(const char *str, char **end, const rgba_t def)
{
	int string_len;
	char *ep;
...
area_t *area_get(xcb_window_t win, const int btn, const int x)
{
	...
	for (int i = area_stack.at - 1; i >= 0; i--) {
		...
		if (a->window == win && a->button == btn && x >= a->begin &&
		    x < a->end)
			return a;
	}
	return NULL;
...
bool area_add(char *str, const char *optend, char **end, monitor_t *mon,
	      const int x, const int align, const int button)
{
	int i;
	char *trail;
	area_t *a;
	...
	if (*str != ':') {
		*end = str;
		...
		for (i = area_stack.at - 1;
		     i >= 0 && !area_stack.area[i].active; i--)
			;
		a = &area_stack.area[i];
	...
	if (area_stack.at + 1 > area_stack.max) {
		fprintf(stderr,
			"Cannot add any more clickable areas (used %d/%d)\n",
...
void parse(char *text)
{
	...
	/* Create xft drawable */
	if (!(xft_draw =
		      XftDrawCreate(dpy, mon->pixmap, visual_ptr, colormap))) {
	...
	for (;;) {
		...
		if (p[0] == '%' && p[1] == '{' &&
			...
			while (p < block_end) {
				...
				case 'T':
					if (*p ==
					    '-') { //Reset to automatic font selection
						font_index = -1;
						p++;
						break;
					} else if (isdigit(*p)) {
						font_index = (int)strtoul(
							p, &ep, 10);
						// User-specified 'font_index' ∊ (0,font_count]
						// Otherwise just fallback to the automatic font selection
						if (!font_index ||
						    font_index > font_count)
							font_index = -1;
						p = ep;
						break;
					} else {
...
void font_load(const char *pattern)
{
	if (font_count >= MAX_FONT_COUNT) {
		fprintf(stderr,
			"Max font count reached. Could not load font \"%s\"\n",
			pattern);
		return;
	}
	...
	if (!xcb_request_check(c, cookie)) {
		...
		if (lut_size) {
			...
			memcpy(ret->width_lut,
			       xcb_query_font_char_infos(font_info), lut_size);
		}
		free(font_info);
...
void add_y_offset(int offset)
{
	if (offset_y_count >= MAX_FONT_COUNT) {
		fprintf(stderr,
			"Max offset count reached. Could not set offset \"%d\"\n",
			offset);
		return;
	}
...
void set_ewmh_atoms(void)
{
	...
	for (int i = 0; i < atoms; i++)
		atom_cookie[i] = xcb_intern_atom(c, 0, strlen(atom_names[i]),
						 atom_names[i]);
	...
	xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window,
			    atom_list[NET_WM_STRUT_PARTIAL], XCB_ATOM_CARDINAL,
			    32, 12, strut);
	xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window,
...
monitor_t *monitor_new(int x, int y, int width, int height)
{
	...
	xcb_create_window(
		c, depth, ret->window, scr->root, ret->x, ret->y, width, bh, 0,
		XCB_WINDOW_CLASS_INPUT_OUTPUT, visual,
		XCB_CW_BACK_PIXEL | XCB_CW_BORDER_PIXEL |
			XCB_CW_OVERRIDE_REDIRECT | XCB_CW_EVENT_MASK |
			XCB_CW_COLORMAP,
		(const uint32_t[]){ bgc.v, bgc.v, dock,
...
void get_randr_monitors(void)
{
	...
	if (!rres_reply) {
		fprintf(stderr,
			"Failed to get current randr screen resources\n");
		return;
	}
	...
	for (i = 0; i < num; i++) {
		...
		oi_reply = xcb_randr_get_output_info_reply(
			c,
			xcb_randr_get_output_info(c, outputs[i],
						  XCB_CURRENT_TIME),
			NULL);

		// Output disconnected or not attached to any CRTC ?
		if (!oi_reply || oi_reply->crtc == XCB_NONE ||
		    oi_reply->connection != XCB_RANDR_CONNECTION_CONNECTED) {
			free(oi_reply);
			continue;
		}
		...
		memcpy(i_mon_name, xcb_randr_get_output_info_name(oi_reply),
		       oi_reply->name_len);
		...
		mon = monitor_new(bx + ci_reply->x, ci_reply->y,
				  min(bw, ci_reply->width), ci_reply->height);
...
void init(char *wm_name, char *wm_instance)
{
	...
	if (!mon) {
		...
		// Check the geometry
		if (bx + bw > scr->width_in_pixels ||
		    by + bh > scr->height_in_pixels) {
			fprintf(stderr,
				"The geometry specified doesn't fit the screen!\n");
			exit(EXIT_FAILURE);
		}
	...
	xcb_create_gc(c, gc[GC_DRAW], mon->pixmap, XCB_GC_FOREGROUND,
		      (const uint32_t[]){ fgc.v });

	gc[GC_CLEAR] = xcb_generate_id(c);
	xcb_create_gc(c, gc[GC_CLEAR], mon->pixmap, XCB_GC_FOREGROUND,
		      (const uint32_t[]){ bgc.v });

	gc[GC_ATTR] = xcb_generate_id(c);
	xcb_create_gc(c, gc[GC_ATTR], mon->pixmap, XCB_GC_FOREGROUND,
		      (const uint32_t[]){ ugc.v });
	...
	xcb_configure_window(c, mon->window,
	...
	while ((ch = getopt(argc, argv, "hg:bdf:a:pu:B:F:U:n:o:")) != -1) {
		...
		case 'h':
			printf("lemonbar version %s patched with XFT support\n",
			       VERSION);
			printf("usage: %s [-h | -g | -b | -d | -f | -a | -p | -n | -u | -B | -F] MONITOR\n"
			       "\t-h Show this help\n"
		...
		case 'B':
			dbgc = bgc =
				parse_color(optarg, NULL, (rgba_t)0x00000000U);
			break;
		case 'F':
			dfgc = fgc =
				parse_color(optarg, NULL, (rgba_t)0xffffffffU);
			break;
		case 'U':
	...
	for (;;) {
		...
		if (poll(pollin, 2, -1) > 0) {
			if (pollin[0].revents & POLLHUP) { // No more data...
				if (permanent)
					pollin[0].fd =
						-1; // ...null the fd and continue polling :D
				else
					break; // ...bail out
			}
			if (pollin[0].revents &
			    POLLIN) { // New input, process it
				input[0] = '\0';
			...
			if (pollin[1].revents &
			    POLLIN) { // The event comes from the Xorg server
		...
		if (redraw) { // Copy our temporary pixmap onto the window
			xcb_copy_area(c, mon->pixmap, mon->window, gc[GC_DRAW],
				      0, 0, 0, 0, mon->width, bh);
		}
